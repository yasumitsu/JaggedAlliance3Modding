...
---
--- Returns a map of groups that the ambient life zone unit belongs to.
---
--- The map is generated by combining the groups from the `zone` property and the `Groups` property of the ambient life zone unit.
---
--- @return table groups_map A map of groups that the ambient life zone unit belongs to.
---
...
---
--- Checks if the ambient life zone unit belongs to any of the groups that the given `other` object belongs to.
---
--- @param other table The object to check the groups against.
--- @param groups_map table|nil A map of groups that the ambient life zone unit belongs to. If not provided, it will be generated using `self:GetGroupsMap()`.
--- @return boolean true if the ambient life zone unit belongs to any of the groups that the `other` object belongs to, false otherwise.
---
...
--- Returns a string representation of the AnimationWeight object for the editor view.
---
--- The string includes the name of the animation and the weight value.
---
--- @return string The editor view string for the AnimationWeight object.
...
---
--- Handles the editor property changes for the `AnimationWeight` class.
---
--- When the `AnimStart`, `Animation`, or `AnimEnd` properties are changed, this function
--- attempts to find the corresponding start, idle, and end animations based on the
--- new property values. If all three animations are found, the properties are updated
--- to match the found animations.
---
...
---
--- Gets a list of animation sets that can be played for the given unit, along with the total chance for each set.
---
--- @param unit table The unit to get the animation sets for.
--- @return table A list of animation sets that can be played, with the following fields:
---   - total_chance: The total chance for this animation set to be played.
---   - AnimStart: The name of the start animation for this set.
---   - Animation: The name of the idle animation for this set.
---   - AnimEnd: The name of the end animation for this set.
---
...
---
--- Gets a random animation set that can be played for the given unit, based on the weighted chances of each set.
---
--- @param unit table The unit to get the random animation set for.
--- @return table|nil The randomly selected animation set, or nil if no sets are available.
---   - total_chance: The total chance for this animation set to be played.
---   - AnimStart: The name of the start animation for this set.
---   - Animation: The name of the idle animation for this set.
---   - AnimEnd: The name of the end animation for this set.
...
---
--- Plays a random animation set for the given unit.
---
--- @param unit table The unit to play the animation set for.
--- @param flags number (optional) Flags to pass to the animation state change.
--- @param crossfade number (optional) Crossfade duration for the animation state change.
--- @return table|nil The randomly selected animation set, or nil if no sets are available.
---   - total_chance: The total chance for this animation set to be played.
---   - AnimStart: The name of the start animation for this set.
---   - Animation: The name of the idle animation for this set.
...
---
--- Returns a list of GridMarker objects that define the routine area for the given unit.
---
--- If the unit's `routine_area` is set to "self", the function will return a list containing
--- the unit's `routine_spawner` object, if it is valid and not an instance of `AL_Football`.
---
--- If the unit's `routine_area` is set to a group name, the function will return a list of
--- all valid `GridMarker` objects in that group.
---
--- If the `routine_area` group is empty or does not exist, an error will be logged.
---
--- @param self Unit
--- @return table|nil markers A list of GridMarker objects defining the routine area, or nil if no markers are found
...
---
--- Returns a random visitable object from the unit's routine area, optionally filtering by a specified type.
---
--- If `low_covers` is true, the function will also search for low cover spots within the unit's routine area that are not reserved and can be occupied by the unit.
---
--- @param self Unit The unit requesting a random visitable
--- @param low_covers boolean If true, the function will also search for low cover spots
--- @param filter function An optional filter function to apply to the visitable objects
--- @param ... any Additional arguments to pass to the filter function
--- @return table|nil A table containing the selected visitable object, or nil if no visitable object is found
...
---
--- Reserves a visitable object for the unit.
---
--- If the visitable object is a cover spot, it marks the cover spot as reserved by the unit.
--- If the visitable object is not a cover spot, it marks the visitable object as reserved by the unit.
---
--- @param visitable table The visitable object to reserve.
...
---
--- Returns the visitable object that is currently reserved by this unit.
---
--- @return table|nil The reserved visitable object, or nil if no visitable is reserved.
function Unit:GetVisitable()
	return table.find_value(g_Visitables, "reserved", self.handle)
end

---
--- Frees a previously reserved visitable object for the unit.
---
--- If the visitable object is a cover spot, it marks the cover spot as no longer reserved by the unit.
--- If the visitable object is not a cover spot, it marks the visitable object as no longer reserved by the unit.
---
--- @param visitable table The visitable object to free.
...
---
--- Teleports the unit to a random low cover spot and sets the unit to the "Cower" command.
---
--- This function first tries to find a random visitable object that is a low cover spot. If a cover spot is found, the unit is teleported to the cover spot and oriented to face the cover spot. If the visitable object is not a cover spot, the unit is teleported to the object's position and oriented to face the object.
---
--- After teleporting the unit, the "Cower" command is set for the unit, and the move animation is set to "Run".
---
--- @param self Unit The unit that is teleporting to a cover spot.
...
---
--- Handles the ambient routine for a unit, which includes roaming, visiting, and standing still behaviors.
---
--- The function first checks if the unit should roam or visit a visitable object. If the unit should roam, it finds a random roam marker and sets the unit's command to "RoamSingle". If the unit should visit, it finds a random visitable object and sets the unit's command to "Visit".
---
--- If the unit cannot find a visitable object to roam or visit, it falls back to a "StandStill" routine, where the unit will play a random idle animation for a period of time.
---
--- @param self Unit The unit that is performing the ambient routine.
...
---
--- Handles the "StandStill" idle routine for a unit.
---
--- This function sets the unit's idle animation style to a random "StandStill" animation, and then plays a series of random animations from that style for a given timeout period. If no timeout is provided, the function will continue playing animations indefinitely.
---
--- @param timeout number (optional) The maximum time in seconds to play the idle animations.
--- @param dont_halt boolean (optional) If true, the function will not halt the game when the animations are finished.
---
...
---
--- Handles the idle routine for a unit. This routine is executed when the unit is not performing any other specific action.
--- The routine checks the unit's status and current routine, and executes the appropriate subroutine based on the unit's state.
--- If the unit has the "Suspicious" status effect, it will execute the SuspiciousRoutine.
--- If the unit's routine is set to "Ambient", it will execute the AmbientRoutine.
--- If the unit's routine is set to "StandStill", it will execute the IdleRoutine_StandStill.
--- If the unit's routine is set to "Patrol", it will execute the patrol routine, which involves following a predefined route.
--- If the unit's routine is set to "AdvanceTo", it will execute the advance to routine, which involves moving the unit to a specified marker.
--- If the unit's routine is unknown, an error will be logged and the unit will sleep for 1000 milliseconds.
---
...
---
--- Retrieves a roam position for a unit based on a given marker.
---
--- If the marker is a point, it is returned as is. Otherwise, the function
--- retrieves a list of positions within the marker's area, filters them based
--- on the unit's zone, and selects the best position that is furthest from the
--- unit's current position and closest to the unit's current orientation.
---
--- @param marker GridMarker|Point The marker or point to get the roam position from.
--- @return Point|nil The roam position, or nil if no suitable position is found.
---
...
---
--- Attaches a flare visual object to the unit's right weapon spot.
--- The flare is muted and the unit is marked as carrying a flare.
--- The unit's outfit is updated to reflect the carried flare.
---
--- @param self Unit The unit attaching the flare.
---
...
---
--- Simulates an object falling down to the ground.
---
--- @param obj Object The object to be fallen down.
---
...
---
--- Detaches a flare visual object from the unit's right weapon spot, places it on the ground, and marks the unit as no longer carrying a flare.
---
--- @param self Unit The unit detaching the flare.
---
...
---
--- Plays a roam animation for the unit based on the provided marker.
---
--- If the unit is a human, is not infected, and is not carrying a flare, there is a 50% chance that the unit will attach a flare and become a flare carrier.
---
--- The unit will then move to a random position around the provided marker and play the appropriate roam animation based on the unit's species and roam animation set.
---
--- If the unit is carrying a flare, it will play the "Idle_Flare" animation style. Otherwise, it will play the animation from the unit's roam animation set.
---
--- @param self Unit The unit playing the roam animation.
--- @param marker Object The marker to use for the roam position.
---
...
---
--- Causes the unit to roam around the provided marker, playing a roam animation.
---
--- If the unit is in combat or the marker is nil, the unit will be set to idle and the behavior will be cleared.
---
--- The unit will be set to a standing stance, and the PlayRoamAnimation function will be called in an infinite loop.
---
--- If an end_orient parameter is provided, the unit's orientation will be set to the angle of the marker after the roam is complete.
---
--- @param marker Object The marker to use as the center of the roam.
--- @param end_orient boolean Whether to set the unit's orientation to the angle of the marker after the roam is complete.
---
...
--- If the unit is in combat or the marker is nil, the unit will be set to idle and the behavior will be cleared.
---
--- If the unit is a Hyena, the unit will be set to the "RoamHyenaLead" command instead of roaming.
---
--- The unit will be set to a standing stance, and the PlayRoamAnimation function will be called.
---
--- @param marker Object The marker to use as the center of the roam.
---
...
---
--- Gets a random move style for the unit based on the provided walk type.
---
--- If a walk type is provided, a random animation style for that walk type is returned.
--- If no walk type is provided, the function checks if the unit is a prostitute and returns a random "SeduceWalk" animation style. If the unit is not a prostitute, a random "Walk" animation style is returned.
---
--- @param walk_type string The type of walk to get a random animation style for.
--- @return string The random animation style for the provided walk type or the default "Walk" style.
---
...
---
--- Resets the move style of the unit.
---
--- This function is used to reset the move style of the unit, which is used to determine the animation style for the unit's movement.
---
...
---
--- Visits a visitable object and performs related actions.
---
--- This function is responsible for handling the visit behavior of a unit. It sets up the necessary state and calls the appropriate methods on the visitable object to perform the visit.
---
--- @param visitable table The visitable object to visit. This should be a table with three elements: a marker, a destination, and a look-at position.
--- @param already_in_perpetual boolean (optional) A flag indicating whether the visit is part of a perpetual behavior.
--- @return nil
---
...
---
--- Checks if the unit is currently visiting a visitable object.
---
--- This function returns true if the unit's current behavior is "Visit", "Roam", or "RoamSingle", and the current behavior matches the unit's command.
---
--- @return boolean true if the unit is currently visiting a visitable object, false otherwise
---
...
---
--- Runs the unit away from a cower position.
---
--- This function generates a random angle to run away from the cower position, ensuring the angle is not too close to the cower angle. It then calculates a destination position based on the random angle and the cower run distance, and moves the unit to that position.
---
--- @param self Unit The unit that is running away from the cower position.
--- @return nil
---
...
---
--- Checks if the unit can change its cower spot.
---
--- This function returns true if the unit is not in combat and the cower cooldown has expired, and a random chance check passes. The chance of changing the cower spot is determined by the `const.AmbientLife.CowerSpotChangeChance` constant.
---
--- @param self Unit The unit that is checking if it can change its cower spot.
--- @return boolean true if the unit can change its cower spot, false otherwise.
---
...
---
--- This function sets the unit's current idle style and plays a sequence of random animations from the idle style, optionally for a specified duration or number of cycles. If the idle style has a defined end animation, it will be played at the end of the sequence.
---
--- @param self Unit The unit that will play the idle animation style.
--- @param idle_style string The name of the idle animation style to play.
--- @param duration number (optional) The duration in seconds for which to play the idle animation style.
--- @param anim_cycles number (optional) The number of animation cycles to play before ending the sequence.
--- @return nil
---
...
---
--- Plays the end animation of the specified idle animation style for the unit.
---
--- This function checks if the unit's current animation matches the start animation or any other animation in the specified idle animation style. If so, it plays the end animation of that style.
---
--- @param self Unit The unit that will play the end animation.
--- @param idle_style string The name of the idle animation style to play the end animation for.
--- @return nil
---
...
---
--- Checks if the unit is allowed to cower.
---
--- @param self Unit The unit to check.
--- @return boolean True if the unit is allowed to cower, false otherwise.
---
...
---
--- Causes the unit to cower in fear.
---
--- This function makes the unit cower in fear for a specified timeout duration. It will find a suitable cower spot, play the cower animation, and restore the unit's previous behavior when the cower duration ends.
---
--- @param self Unit The unit that will cower.
--- @param find_cower_spot boolean If true, the unit will find a new cower spot each time it cowers. If false, it will use the same cower spot.
--- @param timeout number The maximum duration in seconds that the unit will cower for.
--- @param restore_behavior string (optional) The name of the behavior to restore after the cower duration ends.
--- @param restore_behavior_params table (optional) The parameters to restore the previous behavior with.
--- @return nil
---
...
---
--- Advances the unit to a specified marker.
---
--- @param handle string The handle of the marker to advance to.
--- @param delay number|nil The delay in milliseconds before advancing.
---
...
--- Resets the cached route and angle information for marker groups.
---
--- This function clears the global tables `g_MarkerGroupRoute` and `g_MarkerGroupAngle`, which are used to cache the route and angle information for marker groups. This is useful when the marker group data changes and the cached information needs to be invalidated.
---
...
---
--- Gets the route and angle information for a marker group.
---
--- This function retrieves the cached route and angle information for the specified marker group. If the cached information is not available, it calculates the route and angle based on the markers in the group and stores the results in the global tables `g_MarkerGroupRoute` and `g_MarkerGroupAngle`.
---
--- @param marker_group string The name of the marker group to retrieve the route and angle for.
--- @return table, number The route and angle information for the marker group.
...
---
--- Patrols the unit along a route defined by a marker group.
---
--- @param marker_group string The name of the marker group defining the patrol route.
--- @param next_id number The ID of the next marker in the route to move to.
--- @param loop boolean Whether the patrol should loop back to the first marker when reaching the end.
--- @param end_orient boolean Whether the unit should face the direction of the last marker in the route.
---
--- This function sets the unit's behavior to "Patrol" and moves the unit along the route defined by the specified marker group. If the `next_id` parameter is not provided, it starts from the first marker in the route. If `loop` is true, the unit will loop back to the first marker when reaching the end of the route. If `end_orient` is true, the unit will face the direction of the last marker in the route when the patrol is complete.
---
--- The function retrieves the route and angle information for the marker group using the `GetRouteFromMarkerGroup` function. It then moves the unit to each waypoint in the route, playing any associated animation at each waypoint. If the unit cannot reach a waypoint, it will teleport to the closest valid position.
---
--- When the end of the route is reached, the function checks the `loop` parameter. If `loop` is true, it restarts the patrol from the first marker. If `loop` is false and `end_orient` is true, it sets the unit's orientation to the angle of the last marker in the route. Finally, it sets the unit's behavior to "Idle" and its command to "Idle".
---
...
--- Exits the unit from the map.
---
--- @param marker table The marker to teleport the unit to when exiting the map.
--- @param start_time number The time in milliseconds when the unit should start exiting the map.
---
--- This function sets the unit's behavior to "ExitMap" and moves the unit to the specified marker. If no marker is provided, it simply sets the unit's behavior to an empty state.
---
--- If the unit is not an ambient unit and there is time remaining before the start_time, the function sets a random idle animation and sleeps until the start_time is reached.
---
--- The function then changes the unit's stance to "Standing", modifies the object, and pushes a destructor to overwrite the unit's pathfinding class if necessary. If the unit's species is "Human", it overwrites the pathfinding class to the player1 class.
---
--- Finally, the function moves the unit to the marker's position using the specified move animation, pops and calls the destructor, and despawns the unit.
...
---
--- Enters the unit into the game map.
---
--- @param zone table The zone the unit is entering.
--- @param pos table The position the unit should be placed at.
--- @param wait_time number The time in milliseconds the unit should wait before entering the map.
---
--- This function sets up the unit's entry into the map. If there is a conflict or scripted conflict in the game state, the unit is immediately destroyed. Otherwise, the function sets the unit's `enter_map_wait_time` and `enter_map_pos` properties, and pushes a destructor to reset these properties if the unit is destroyed.
---
--- If a `wait_time` is provided, the function sleeps for that duration before continuing. It then pops and calls the destructor, and if the unit is still valid, it pushes another destructor to overwrite the unit's pathfinding class if necessary. If the unit's species is "Human", it overwrites the pathfinding class to the "player1" class.
---
--- The function then sets the unit's command parameter "move_anim" to "Walk", moves the unit to the specified position, and pops and calls the destructor.
---
--- Finally, the function checks the game state again. If there is a conflict or scripted conflict, it sets the unit's command to "Cower" with a "Run" move animation. Otherwise, it calls the unit's `AmbientRoutine()` function.
...
---
--- Moves the unit back to the specified position and resets its behavior.
---
--- @param pos table The position to move the unit to.
---
...
---
--- Checks if the unit is an ambient unit.
---
--- @return boolean true if the unit is an ambient unit, false otherwise
---
...
---
--- Checks if the unit is a prostitute.
---
--- @return boolean true if the unit is a prostitute, false otherwise
---
...
---
--- Resets the ambient life behavior of the unit.
---
--- @param kick_perpetual_units boolean If true, forces the unit to exit any perpetual markers.
--- @param force_immediate_kick boolean If true, immediately clears the perpetual marker instead of waiting for the next frame.
---
...
---
--- Places a corpse marker of the specified class at a random position around the unit.
---
--- @param class string The class name of the corpse marker to place.
--- @param visit_min number The minimum duration that the marker can be visited.
--- @param visit_max number The maximum duration that the marker can be visited.
--- @return AL_CorpseMarker|nil The placed corpse marker, or nil if placement failed.
---
...
---
--- Places corpse markers for a dead unit.
---
--- This function attempts to place two corpse markers around the dead unit - one for mourning and one for marauding. The markers are placed at random positions around the unit, and their visit durations are randomized within a specified range.
---
--- If the placement of either marker fails, the function will try up to 10 times before giving up.
---
--- @param self Unit The dead unit.
---
...
---
--- Removes the ambient life dead markers associated with this unit.
---
--- This function removes the "mourn" and "maraud" ambient life markers that were previously placed for this dead unit. It kicks out any visitors from the markers and then deletes the markers.
---
--- @param self Unit The dead unit.
---
...
---
--- Fixes up ambient life dead markers in a sector's spawn data.
---
--- This function processes the spawn data for a sector, looking for any "AL_Mourn_FromCorspe" or "AL_Maraud_FromCorspe" markers that do not have a corresponding corpse. It then associates those markers with the closest dead unit in the sector.
---
--- @param sector_data table The sector data, containing the spawn data.
--- @param lua_revision number The current Lua revision.
--- @param handle_data table The handle data, containing information about each spawned object.
---
