...
---
--- Generates a colonial heavy style decoration using various procedural generation functions.
---
--- @param seed number|nil The seed to use for the random number generator. If not provided, a random seed will be used.
--- @param initial_selection table|nil The initial selection to use as a starting point for the generation. If not provided, the current editor selection will be used.
---
function ExtrasGenPrgs.ColonialHeavy(seed, initial_selection)
    -- Function implementation
end
ExtrasGenPrgs.ColonialHeavy = function(seed, initial_selection)
	local li = { id = "ColonialHeavy" }
	...
	prgdbg(li, 1, 11) sprocall(ExtrasGenPrgs.LayColonialEdge_04, rand(), nil)
	prgdbg(li, 1, 12) local guides = editor.GetSel()
	prgdbg(li, 1, 13) sprocall(MoveSizeGuides.Exec, MoveSizeGuides, guides, "m", 0, "m", 700, "m", 0, false)
	prgdbg(li, 1, 14) sprocall(ExtrasGenPrgs.LayColonialWallColumn_01, rand(), nil)
	prgdbg(li, 1, 15) sprocall(ExtrasGenPrgs.WallEdgesRightToLeft, rand(), initial_selection)
	prgdbg(li, 1, 16) guides = editor.GetSel()
	prgdbg(li, 1, 17) sprocall(MoveSizeGuides.Exec, MoveSizeGuides, guides, "m", 0, "m", 700, "m", 0, false)
	prgdbg(li, 1, 18) sprocall(ExtrasGenPrgs.LayColonialWallColumn_01, rand(), nil)
